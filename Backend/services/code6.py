...
def process_video_tamper(session_id: str, input_path, mongo_credentials: dict, frame_interval: int = 5):
    ...
    # Output video setup
    output_video = f"output_videos/tamper_detection_{session_id}.mp4"
    ...
    try:
        while True:
            if stop_processing:
                print("Stopping tampering detection process...")
                break
            ...
            # Detect blur
            blur_flag, blur_score = detect_blur(frame, blur_threshold)
            if blur_flag:
            ...
            # Detect scene change
            scene_change_flag, scene_score = detect_scene_change(prev_frame, frame, scene_change_threshold)
            ...
            # Occlusion detection
            occlusion_flag = detect_occlusion(frame, occlusion_threshold)
            if occlusion_flag:
            ...
            # Display tampering alerts on frame
            if alert_status:
                y = 50
                for t_type, active in tamper_state.items():
                    if active:
...
def detect_blur(frame, threshold=180):
    """Detect if the frame is blurry using Laplacian variance."""
...
def detect_occlusion(frame, threshold=100):
    """Detect if the camera is occluded based on histogram variance."""
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    hist = cv2.calcHist([gray], [0], None, [256], [0, 256])
    hist_var = np.var(hist)
    return hist_var < threshold
